...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        ###     Dropout Layer:
        ###         https://pytorch.org/docs/stable/generated/torch.nn.Dropout.htm
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # X's shape is (src_len, b, e), source_padded's shape is (src_len, b)
        X = self.model_embeddings.source(source_padded)
        
        # Change tensor's shape to (b, e, src_len) and feed it to conv1d
        X = X.permute(1, 2, 0)
        X = self.post_embed_cnn(X)

        # conv1d will preserve it's shape and change tensor's shape back to (src_len, b, e)
        X = X.permute(2, 0, 1)

        # use pack_padded_sequence to ignore padding tokens when calculating
        packed_input_to_encoder = pack_padded_sequence(X, source_lengths, enforce_sorted=False)

        # apply the encoder, the shape of packed_output is (src_len, b, 2*h) and the shape of h_m or c_m is (2, b, h)
        packed_output, (h_m, c_m) = self.encoder(packed_input_to_encoder)

        # use pad_packed_sequence to unpack the enc_hiddens
        output, _ = pad_packed_sequence(packed_output)

        # change the shape of output from (src_len, b, 2*h) to (b, src_len, 2*h)
        enc_hiddens = output.permute(1, 0, 2)

        # reshape the h_m and c_m from (2, b, h) to (b, 2h)
        b, h = h_m.shape[1], h_m.shape[2]
        h_m = h_m.permute(1, 0, 2).reshape(b, 2*h)
        c_m = c_m.permute(1, 0, 2).reshape(b, 2*h)
        
        # calculate the init decoder hidden & init_decoder_cell
        init_decoder_hidden = self.h_projection(h_m)
        init_decoder_cell = self.c_projection(c_m)

...
if args.variant == 'vanilla':
    ...
    # # use when training
    # model = models.GPT(mconf).to('mps')

    # use when evaluating
    model = models.GPT(mconf).to(device)
    # print(next(model.parameters()).device)
    # print(model.rope_cache.device)
...
if args.function == 'pretrain':
    ...
    ### YOUR CODE HERE ###
    pretraining_conf = trainer.TrainerConfig(
        max_epochs=650,
        batch_size=128,
        learning_rate=args.pretrain_lr,
        lr_decay=True,
    ...
    trainer = trainer.Trainer(model, pretrain_dataset, None, pretraining_conf)
    trainer.train()
    ...
    pretrained_model_path = args.reading_params_path
    save_finetuned_model_path = args.writing_params_path
    finetune_text = open(finetune_corpus_path, 'r', encoding='utf-8').read()

    if pretrained_model_path is not None:
        model.load_state_dict(torch.load(pretrained_model_path))
        train_conf = train_conf_with_pretraining = trainer.TrainerConfig(
            max_epochs=10,
            batch_size=256,
            learning_rate=args.finetune_lr,
            lr_decay=True,
        ...
        train_conf = trainer.TrainerConfig(
            max_epochs=75,
            batch_size=256,
            learning_rate=args.finetune_lr,
            lr_decay=True,
    ...
    finetuning_dataset = dataset.NameDataset(pretrain_dataset, finetune_text) 
    trainer = trainer.Trainer(model, finetuning_dataset, None, train_conf)
    trainer.train()

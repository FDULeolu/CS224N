...
def precompute_rotary_emb(dim, max_positions):
    ...
    # TODO: [part g]
    ### YOUR CODE HERE ###
    theta_i = [1/(10000 ** (-2*i/dim)) for i in range(dim // 2)]
    cos = []
    sin = []
    for t in range(max_positions):
        cos_t = [math.cos(t * i) for i in theta_i]
        sin_t = [math.sin(t * i) for i in theta_i]
        cos.append(cos_t)
        sin.append(sin_t)
    rope_cache = torch.tensor([cos, sin])
    rope_cache = torch.permute(rope_cache, (1, 2, 0))
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    # use when evaluating
    rope_cache = rope_cache[0:length, : , :].contiguous()
    rope_cache_complex = torch.view_as_complex(rope_cache)
    batch_rope_cache_complex = rope_cache_complex.unsqueeze(0).expand(batch, -1, -1).unsqueeze(1).expand(-1, head, -1, -1)
    x = x.reshape(batch, head, length, dim//2, 2)
    x_complex = torch.view_as_complex(x)
    rotated_x_complex = x_complex * batch_rope_cache_complex
    rotated_x_real = torch.view_as_real(rotated_x_complex)
    rotated_x = torch.sum(rotated_x_real, dim=-1)

    # use when training
    # # move x from mps to cpu to calculate complex number
    # x_cpu = x.cpu()
    # rope_cache_cpu = rope_cache.cpu()

    # rope_cache_cpu = rope_cache_cpu[0:length, :, :].contiguous()
    # rope_cache_complex = torch.view_as_complex(rope_cache_cpu)
    # batch_rope_cache_complex = rope_cache_complex.unsqueeze(0).expand(batch, -1, -1).unsqueeze(1).expand(-1, head, -1, -1)
    # x_cpu = x_cpu.view(batch, head, length, dim // 2, 2)
    # x_complex = torch.view_as_complex(x_cpu)
    # rotated_x_complex = x_complex * batch_rope_cache_complex
    # rotated_x_real = torch.view_as_real(rotated_x_complex)
    # rotated_x = torch.sum(rotated_x_real, dim=-1)

    # # move the result back to mps
    # rotated_x = rotated_x.to('mps')

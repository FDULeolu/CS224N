...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.tensor(np.zeros((self.embed_size * self.n_features, self.hidden_size)), dtype=torch.float)))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.tensor(np.zeros((self.hidden_size)), dtype=torch.float)))
        self.dropout = nn.Dropout(dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(torch.tensor(np.zeros((self.hidden_size, self.n_classes)), dtype=torch.float)))
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(torch.tensor(np.zeros((self.n_classes)), dtype=torch.float)))
    ...
    def forward(self, w):
        ...
        x = torch.matmul(embeddings, self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        relu = nn.ReLU()
        x = relu(x)
        x = self.dropout(x)
